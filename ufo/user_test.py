...
FAKE_USER = MagicMock(id=FAKE_DS_KEY, email=FAKE_EMAIL,
...
SQLALCHEMY_DATABASE_URI = 'sqlite:///random_test_db.db'
TESTING = True
...
class UserTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    # database.Add(mock_config)
    # self.session.delete(database.GetFirstRecord(models.Config))
    # self.session.commit()
    self.session.add(self.mock_config)
    self.session.commit()


    # self.db_fd, management_server.app.config['DATABASE'] = tempfile.mkstemp()
    # self.app = management_server.app.test_client()
    # management_server.db.init_db(self.app)
    # management_server.db.create_all()
    # management_server.init_db()
  ...
  #   os.close(self.db_fd)
  #   os.unlink(SQLALCHEMY_DATABASE_URI)
  ...
  #   self.app.get(PATHS['landing_page_path'])
  ...
  def testListUsersHandler(self):
    ...
    # database.Add(fake_user)
    self.session.add(fake_user)
    self.session.commit()
    user_id = fake_user.id

    resp = self.app.get(PATHS['user_page_path'])
    ...
    self.assertEquals(FAKE_EMAIL in user_list_output, True)
    details_link = ('/user/' + str(user_id) + '/details')
